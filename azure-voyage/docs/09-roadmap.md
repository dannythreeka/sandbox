...
原則：**每個里程碑結束時，遊戲都處於「可運行、可驗證」狀態**。每個 M 是一個或多個 PR；驗收標準必須全綠才進下一個 M。全程 `AI_ENABLED=false` 也必須可玩（AI 里程碑才開 AI）。

> **執行記錄**：實際實作時把原本的 M4（"完整六系統"）拆得更細，改為 M2–M9 共 8 個里程碑、每個獨立 PR、逐一 squash-merge 進 `main`：M2 航行迴圈、M3 經濟貿易、M4 航海士與造船廠、M5 海戰、M6 事件與探索、M7 NPC 商會與勝敗、M8 AI Agent 層、M9 打磨與可玩發布。以下 M2 之後的章節內容維持原設計意圖，實際 PR 範圍與順序以此執行記錄為準。
