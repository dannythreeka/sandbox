...
> **執行記錄**：M11–M14 全數完成，設計與下方規格大致一致，實務調整：
...
> - M14 前先修掉一個 M11 遺留的重複實作：`wind.ts` 的 `regionAt` 其實與
>   `content/regions.ts` 既有的 `regionForCoord` 一模一樣（M11 開發時沒搜就重寫了一次）；
>   已刪除 `regionAt`，全部呼叫端改用 `regionForCoord`。
> - M14：波光用「固定散佈點＋整層透明度脈動」取代逐點動畫（同樣的視覺效果，成本趨近
>   於零）；風紋只取「艦隊當前所在海域」的單一風向套用到整個可視範圍（而非逐格查詢
>   風向），對海域尺度而言是合理近似。霧／風暴壓暗／雨絲／風暴事件閃光皆為螢幕座標
>   overlay（不隨地圖縮放平移），比對照海圖本身的 Pixi 相機做濾鏡簡單很多。
