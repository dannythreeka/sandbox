...
function makePrisma(overrides: { fleetActivity?: string; lore?: number; alreadyFound?: string[] } = {}) {
  ...
  const discoveryRecords: { id: string; worldId: string; discoveryId: string; registered: boolean }[] =
  ...
  const prisma = {
    ...
    discoveryRecord: {
      ...
      create: jest.fn(async ({ data }: { data: { discoveryId: string; foundTick: number } }) => {
        const record = { id: `dr${discoveryRecords.length}`, worldId: "w1", registered: false, ...data };
      ...
      update: jest.fn(async ({ where, data }: { where: { id: string }; data: Partial<{ registered: boolean }> }) => {
